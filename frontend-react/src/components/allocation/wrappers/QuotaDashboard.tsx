...
export function QuotaDashboard({ enabled = true }: { enabled?: boolean }) {
  ...
  const lifetime = useMemo(() => {
    ...
    nisaGrowth?.lifetime_remaining,
    nisaGrowth?.lifetime_used,
    nisaTsumitate?.lifetime_remaining,
    nisaTsumitate?.lifetime_used,
  ])
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between gap-2">
          ...
          <Badge variant="outline">
            {t(`wrapper.dashboard.policy_${quotaData?.restoration_policy ?? "next_year"}`)}
          </Badge>
        </div>
      </CardHeader>
              ...
              <TooltipTrigger asChild>
                <button
                  ...
                  aria-label={t("wrapper.dashboard.progress_tooltip", {
                    ...
                    remaining: Math.round(Math.max(0, lifetime.remaining)).toLocaleString(),
                    limit: Math.round(
                      Math.max(0, lifetime.used + lifetime.remaining),
                    ).toLocaleString(),
                  })}
                >
                  <div
                ...
                <span className="text-muted-foreground">
                  {t("wrapper.dashboard.used_remaining", {
                    ...
                    remaining: Math.round(
                      Math.max(0, nisaGrowth.growth_sub_limit_remaining ?? 0),
                    ).toLocaleString(),
                  })}
                </span>
              </div>
                  ...
                  <button
                    ...
                    aria-label={t("wrapper.dashboard.progress_tooltip", {
                      ...
                      limit: Math.round(
                        Math.max(
                          0,
                          (nisaGrowth.growth_sub_limit_used ?? 0) +
                            (nisaGrowth.growth_sub_limit_remaining ?? 0),
                        ),
                      ).toLocaleString(),
                    })}
                  >
            ...
            {restorationData?.pending?.length ? (
              <ul className="space-y-1">
                {restorationData.pending.map((item, index) => (
                  <li
                    key={`${item.tax_wrapper}-${item.effective_date}-${index}`}
                    className="text-xs text-muted-foreground"
                  >
                    {t(`wrapper.${item.tax_wrapper}`)}: +{Math.round(item.amount).toLocaleString()}{" "}
                    ({formatDate(String(item.effective_date), i18n.language)})
                  </li>
                ))}
              </ul>
            ) : (
              <p className="text-xs text-muted-foreground">
                {t("wrapper.dashboard.restoration_empty")}
              </p>
            )}
          </div>
        </TooltipProvider>
