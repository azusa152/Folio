...
export function assertEnrichedStocks(data: unknown): EnrichedStock[] {
  if (!Array.isArray(data))
    throw new Error("assertEnrichedStocks: expected array")
  if (data.length > 0) {
    const first = data[0] as Record<string, unknown>
    if (typeof first.ticker !== "string" || typeof first.current_price !== "number")
      throw new Error("assertEnrichedStocks: unexpected element shape (missing ticker or current_price)")
  }
...
export function assertRadarEnrichedStocks(data: unknown): RadarEnrichedStock[] {
  if (!Array.isArray(data))
    throw new Error("assertRadarEnrichedStocks: expected array")
  if (data.length > 0) {
    const first = data[0] as Record<string, unknown>
    if (typeof first.ticker !== "string" || typeof first.category !== "string")
      throw new Error("assertRadarEnrichedStocks: unexpected element shape (missing ticker or category)")
  }
...
export function assertPricePoints(data: unknown): PricePoint[] {
  if (!Array.isArray(data))
    throw new Error("assertPricePoints: expected array")
