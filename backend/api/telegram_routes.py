...
from api.schemas import MessageResponse, TelegramSettingsRequest, TelegramSettingsResponse
...
@router.get("/settings/telegram", response_model=TelegramSettingsResponse, summary="Get Telegram settings")
...
@router.put("/settings/telegram", response_model=TelegramSettingsResponse, summary="Update Telegram settings")
...
@router.post("/settings/telegram/test", response_model=MessageResponse, summary="Send a test Telegram message")
def test_telegram_message(
    ...
    if not settings or not settings.telegram_chat_id:
        raise HTTPException(
            status_code=400,
            detail={"error_code": ERROR_TELEGRAM_NOT_CONFIGURED, "detail": "尚未設定 Telegram Chat ID，請先儲存設定。"},
        )
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail={"error_code": ERROR_TELEGRAM_SEND_FAILED, "detail": f"發送失敗：{e}"}) from e
